...
public interface OrderItemRepository extends ReactiveCrudRepository<OrderItem, Long>, OrderItemRepositoryInternal {
    ...
    @Query("SELECT * FROM order_item entity WHERE entity.ticket_id = :id")
    Flux<OrderItem> findByTicket(Long id);

    @Query("SELECT * FROM order_item entity WHERE entity.ticket_id IS NULL")
    Flux<OrderItem> findAllWhereTicketIsNull();
